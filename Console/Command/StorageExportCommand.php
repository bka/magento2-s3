...
class StorageExportCommand extends \Symfony\Component\Console\Command\Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $destinationModel = $this->coreFileStorage->getStorageModel(\Thai\S3\Model\MediaStorage\File\Storage::STORAGE_MEDIA_AZURE);

        if (!$destinationModel->credentialsValid()) {
        ...
        while (($files = $sourceModel->exportFiles($offset, 1)) !== false) {
            ...
            foreach ($files as $file) {
                $output->writeln(sprintf('Uploading %s to use S3.', $file['filename']));
            }
        ...
        echo "export";
        $storage = ['type' => 3];
        $this->storage->synchronize($storage);
    }
